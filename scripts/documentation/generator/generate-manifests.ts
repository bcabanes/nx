...
export function generateManifests(workspace: string): Promise<void[]> {
  ...
  fileGenerationPromises.push(
    generateJsonFile(resolve(targetFolder, `tags.json`), JSON.stringify(tags))
  );
  fileGenerationPromises.push(
    generateJsonFile(resolve(targetFolder, `menus.json`), JSON.stringify(menus))
  );
        ...
        Object.values(item.documents).forEach((documentMetadata) => {
          documentMetadata.tags.forEach((t: string) => {
            const tagData = {
              ...
              path: ['packages', documentMetadata.path].join('/'),
            };
            !tags[t] ? (tags[t] = [tagData]) : tags[t].push(tagData);
          });
        });
