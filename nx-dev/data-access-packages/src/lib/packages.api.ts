...
export class PackagesApi {
  ...
  getStaticDocumentPaths(): StaticDocumentPaths[] {
    ...
    const packages = Object.keys(this.manifest).map((k) => this.manifest[k]);
    const segments: StaticDocumentPaths[][] = [];
    packages.forEach((p) => {
      segments.push([generateSegments(p.path, this.options.prefix)]);
      segments.push(
      ...
      segments.concat(
        Object.keys(p.executors).map((path) =>
          generateSegments(path, this.options.prefix)
        )
      );
      segments.concat(
        Object.keys(p.generators).map((path) =>
          generateSegments(path, this.options.prefix)
        )
      );
    });
